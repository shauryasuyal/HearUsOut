...
### ⚙️ Heuristic Adaptive Preprocessing Engine
...
> [!TIP]
> **Audio Clipping Fix:** Standard separation models normalize outputs to a standard deviation of 1.0, resulting in peaks of up to 4.0. If saved directly to WAV, this causes severe metallic clipping. Our pipeline natively peak-normalizes all output tensors to `0.95` before disk writes to guarantee pristine audio quality.

> [!NOTE]
> **DeepFilterNet Integration:** The codebase is fully architected to support generative AI refinement via `DeepFilterNet` (to clean up faint robotic separation artifacts). This can be toggled on in local environments by installing the `df` package.

---

## 🛠️ Built With
- **SR_CorrNet** (Core Separation Engine)
- **SpeechBrain / ECAPA-TDNN** (Targeted Voice Extraction)
- **PyTorch & Torchaudio**
- **Gradio Client** (API Routing)
